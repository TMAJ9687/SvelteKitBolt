<script lang="ts">
  ...
  async function handleDowngrade(user: ChatUser) {
    try {
      const userRef = doc(firestore, 'profiles', user.uid);
      await updateDoc(userRef, {
        isVip: false
      });
      toast.success(`${user.nickname} has been downgraded`);
    } catch (error) {
...
<div class="max-w-7xl mx-auto">
  <div class="mb-8">
    <h2 class="text-2xl font-bold mb-2">VIP Users</h2>
    <p class="text-gray-600 dark:text-gray-400">
      Manage VIP users and their permissions
    </p>
  </div>
